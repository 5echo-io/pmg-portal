...
Project: PMG Portal
Purpose: Internationalization (i18n) – how to add and translate strings
Path: docs/I18N.md
Created: 2026-02-05
...
3. Compile messages (from the **`src`** directory):
   ...
   ```
   This creates/updates `locale/nb/LC_MESSAGES/django.mo` from `django.po`. **Norwegian will not appear until the .mo file exists.** Restart the app if it is running. On the server, GNU gettext (`msgfmt`) must be installed; the install and update scripts install the `gettext` package on Debian/Ubuntu.
